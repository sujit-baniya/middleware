...
type ConfigCsrf struct {
	...
	// ErrorHandler is executed when an error is returned from utils.Handler.
	//
...
// default ErrorHandler that process return error from utils.Handler
...
func Csrf(config ...ConfigCsrf) http.HandlerFunc {
	...
	return func(c http.Context) (err error) {
		...
		switch c.Method() {
		case utils.MethodGet, utils.MethodHead, utils.MethodOptions, utils.MethodTrace:
...
func newManager(str contractStorage.Storage) *manager {
